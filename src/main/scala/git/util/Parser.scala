...
object Parser {
  ...
  /** Tells whether the given string is a valid Git safe string.
    ...
    * '.' (0x2e), ',' (0x2c), ':' (0x3a), ';' (0x3b), '<' (0x3c),
	  * '>' (0x3e), '"' (0x22), "'" (0x27).
    */
  def isValidGitSafeString(value: String): Boolean = {
    val onlyInMiddle = Seq(0x20, 0x2e, 0x2c, 0x3a, 0x3b, 0x3c, 0x3e, 0x22, 0x27)

    if (Seq(0x00, 0x0a, 0x3c, 0x3e).exists((c) => value.indexOf(c.toChar) != -1)) false
    else Seq(value.charAt(0), value.charAt(value.length -1)).exists((c) => onlyInMiddle.indexOf(c) != -1)
  }
}